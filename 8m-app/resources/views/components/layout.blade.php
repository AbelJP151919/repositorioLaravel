...
<body class="bg-gray-100">
    ...
    @auth
        <form method="POST" action="/logout">
            @csrf
            <button class="bg-white text-blue-600 px-4 py-2 rounded hover:bg-gray-100 text-sm font-semibold">
                Cerrar sesión
            </button>
        </form>
    @else
        <a href="/login" class="text-white hover:text-gray-200 text-sm">Iniciar Sesión</a>
        <a href="/register" class="bg-white text-blue-600 px-4 py-2 rounded hover:bg-gray-100 text-sm font-semibold">
            Registrarse
        </a>
    @endauth
</div>
